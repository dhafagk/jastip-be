...
export const list = async (req: AuthRequest, res: Response<ApiResponse>, next: NextFunction): Promise<void> => {
  try {
    const data = await productService.listProducts(req.query as ListProductsQuery)
...
export const create = async (req: AuthRequest, res: Response<ApiResponse>, next: NextFunction): Promise<void> => {
  try {
    ...
    const product = await productService.createProduct(req.body as CreateProductInput, sellerId)
...
export const update = async (req: AuthRequest, res: Response<ApiResponse>, next: NextFunction): Promise<void> => {
  try {
    if (!req.user) {
...
export const remove = async (req: AuthRequest, res: Response<ApiResponse>, next: NextFunction): Promise<void> => {
  try {
    if (!req.user) {
      next(new AppError(401, 'Unauthorized.'))
      return
    }
    await productService.deleteProduct(req.params['id'] as string, req.user.sellerId ?? req.user.id, req.user.role)
...
export const addReview = async (req: AuthRequest, res: Response<ApiResponse>, next: NextFunction): Promise<void> => {
  try {
    ...
    const product = await productService.addReview(req.params['id'] as string, req.body as AddReviewInput, req.user.id)
